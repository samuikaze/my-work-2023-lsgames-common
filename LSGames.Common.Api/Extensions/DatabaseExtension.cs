...
namespace LSGames.Common.Api.Extensions
{
    public class DatabaseExtension
    {
        public static IServiceCollection AddDatabaseContext(IServiceCollection serviceCollection, IConfiguration config)
        {
            ...
            string? connectionString = config.GetConnectionString("LSCommons");
            string? username = config.GetValue<string>("Credentials:LSCommons:Username");
            string? password = config.GetValue<string>("Credentials:LSCommons:Password");
            ...
            serviceCollection.AddDbContext<LsgamesCommonsContext>(options =>
